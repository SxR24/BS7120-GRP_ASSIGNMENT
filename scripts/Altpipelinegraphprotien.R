# =====================================================================================
# Alt protien coding pipeline: subpopulation assignment to fetal and organoid clusters
...
if (tsne_success) {
  ...
  # Cluster colours (hue palette)
  n_clusters <- length(levels(plot_df$cluster))
  cluster_colors <- scales::hue_pal()(n_clusters)
  
  # Split data for different point aesthetics (whole vs. micro)
...
# =============================================================================
npc_file <- "protein_Sample_lists_Neurons_vs_NPCs.tsv"
if (file.exists(npc_file)) {
  ...
  if (length(npc_cells) >= 10) {
    ...
    saveRDS(npc_obj, file = "npc_subset_annotated.rds")
...
saveRDS(fetal_obj, file = "fetal_annotated.rds")
saveRDS(organoid_obj, file = "organoid_annotated.rds")
...
write.csv(as.data.frame(pc_lists_padded), 
          "Figure_1C_gene_lists_by_PC_fetal.csv",
          row.names = FALSE, na = "")
...
write.table(data.frame(Sample = colnames(fetal_obj),
                       ...
                       stringsAsFactors = FALSE),
            file = "altpipeline_protien_cell_assignments.txt",
...
fetal_obj$plot_group[fetal_obj$seurat_clusters %in% c(2, 6)] <- "N3"
fetal_obj$plot_group[fetal_obj$seurat_clusters == 4]      <- "N1"
fetal_obj$plot_group[fetal_obj$seurat_clusters %in% c(3, 5)] <- "N2"
...
subpop_df <- data.frame(sample_id = colnames(fetal_obj),
                        group = fetal_obj$plot_group,
                        stringsAsFactors = FALSE)
write.csv(subpop_df, "Altprotiensubpopulations.csv", row.names = FALSE, quote = FALSE)
