# --------------------------------------------------------------------------------------
# Compare multiple PCA gene list files against original and plot overlap percentages
# --------------------------------------------------------------------------------------
library(ggplot2)
library(dplyr)
library(tidyr)
...
# compute overlap for one test file
compute_overlap <- function(test_path, test_name, original_df) {
  ...
  # If no common columns, return an empty data frame
  if (length(common_cols) == 0) {
    warning(paste("No common columns for", test_name))
    return(data.frame(
      TestDataset = character(),
      PC = character(),
  ...
  for (col in common_cols) {
    ...
    pct <- if (length(test_genes) > 0) {
      length(overlap) / length(test_genes) * 100
    } else {
      NA
    }
    ...
    results <- rbind(results, data.frame(
      ...
      PC = col,
      Original_Count = length(orig_genes),
      Test_Count = length(test_genes),
...
# Compute overlap for all test files
...
write.csv(all_results,
          "PCA_overlap_results_all.csv",
          row.names = FALSE)


# Write non‑overlap genes for "Original" dataset only (Optional, not used in report due to word count)
...
# Rename PC labels for readability
...
# Create dodged bar plot as variable in case of saving
