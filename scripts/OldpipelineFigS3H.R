...
# Inputs
expr_file <- "D:/GroupA_University2026_Project/PCA_Output/2026_UoL_prj_gene_matrix_f.tsv"
expr <- read.delim(expr_file, row.names = 1, check.names = FALSE)

meta_file <- "D:/GroupA_University2026_Project/Organoidclusters.csv"
meta <- read.csv(meta_file, stringsAsFactors = FALSE, header = TRUE)
...
block_order <- names(marker_sets)   # order of gene blocks in the heatmap


# Parse sample clusters from metadata
sample_clusters <- list()
for (col in names(meta)) {
  samples <- meta[[col]][!is.na(meta[[col]]) & meta[[col]] != ""]
  sample_clusters[[col]] <- samples
}

# Map sample ID to cluster name
...
# Row clustering within each cluster
row_order <- c()
for (block in block_order) {
...
group_definitions <- list(
  Group1 = c("C2", "C3", "C8", "C10"),
  Group2 = c("C1", "C4", "C7", "C9"),
  Group3 = c("C6"),
  Group4 = c("C5")
)

# Flatten groups into a vector, removing duplicates
group_order <- c()
used_clusters <- c()
...
# Build a vector mapping sample ID to its cluster
new_col_order <- c()
...
# Ensure all columns are included (some samples might have been missed)
...
# Reorder matrix columns and side colours
...
# Manual colours for sample clusters
...
# Add gene block labels next to the row side colour bar
...
# Add legends with proper numeric ordering of clusters
...
sample_cluster_colours_sorted <- cluster_colour_manual[sorted_clusters]   # use the full manual palette
